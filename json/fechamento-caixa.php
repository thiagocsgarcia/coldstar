...
date_default_timezone_set('America/Sao_Paulo');
	...
	switch ($opcao) {
			...
			case 'buscarDia':
				...
				//echo "SELECT * FROM dias WHERE DATE_FORMAT(data, '%d/%m/%Y') = '$hoje' LIMIT 1" ;

				$dia = $time4bd->fetchAll("SELECT * FROM dias WHERE DATE_FORMAT(data, '%d/%m/%Y') = '$hoje' LIMIT 1");
				
				echo json_encode($dia, JSON_PRETTY_PRINT);

			break;

			case 'buscarComprasPorCodigoDia':

				$codigoDia = $_POST['codigoDia'];
			
				$dia = $time4bd->fetchAll("SELECT * FROM compras WHERE codigoDia = '$codigoDia'");
				
				echo json_encode($dia, JSON_PRETTY_PRINT);

			break;

			case 'buscarSangriasPorCodigoDia':

				$codigoDia = $_POST['codigoDia'];
			
				$dia = $time4bd->fetchAll("SELECT * FROM sangrias WHERE codigo = '$codigoDia'");
